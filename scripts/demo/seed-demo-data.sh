...
VALUES
  ...
  ('OBS-EW-002','EW-STATION-03','EW_SIGINT',now() - INTERVAL 47 HOUR,'TRK-0001',
   60.10,-9.74,100,0,0,0.81,'UNCLASSIFIED',
  ...
  ('OBS-EL-001','ELINT-ARRAY-01','ELINT_COMINT',now() - INTERVAL 44 HOUR,'TRK-0005',
   57.50,-10.20,-50,0,0,0.72,'UNCLASSIFIED',
  ...
  ('OBS-ISR-001','ISR-UAV-ALPHA','ISR',now() - INTERVAL 36 HOUR,'TRK-0006',
   56.80,-8.90,0,15.0,90.0,0.95,'UNCLASSIFIED',
  ...
  ('OBS-AIS-001','AIS-COAST-01','AIS_BFT',now() - INTERVAL 48 HOUR,'TRK-0002',
   60.50,-9.00,0,14.2,215.0,0.99,'UNCLASSIFIED',
  ...
  ('OBS-AIS-002','AIS-COAST-01','AIS_BFT',now() - INTERVAL 47 HOUR,'TRK-0002',
   60.38,-9.10,0,14.0,216.0,0.99,'UNCLASSIFIED',
   '{\"mmsi\":\"123456789\",\"vessel_name\":\"ALLIED-VESSEL-1\",\"vessel_type\":\"MILITARY\",\"nav_status\":\"UNDERWAY\"}'),
  ('OBS-AIS-003','AIS-COAST-01','AIS_BFT',now() - INTERVAL 46 HOUR,'TRK-0001',
   61.50,-10.20,0,12.5,180.0,0.45,'UNCLASSIFIED',
...
VALUES
  ('TRK-0001','SURFACE','SUSPECT','ACTIVE',now() - INTERVAL 46 HOUR,
   59.98,-9.75,0,28.5,182.0,0.82,4,'UNCLASSIFIED','USV-SUSPECT-001'),
  ('TRK-0002','SURFACE','FRIENDLY','ACTIVE',now() - INTERVAL 47 HOUR,
   60.38,-9.10,0,14.0,216.0,0.95,2,'UNCLASSIFIED','ALLIED-VESSEL-1'),
  ('TRK-0008','SURFACE','HOSTILE','ACTIVE',now() - INTERVAL 12 HOUR,
   58.50,-11.20,0,32.1,90.0,0.91,3,'UNCLASSIFIED','FAST-MOVER-001'),
  ('TRK-0010','SURFACE','UNKNOWN','ACTIVE',now() - INTERVAL 6 HOUR,
   57.80,-10.50,0,8.5,270.0,0.83,2,'UNCLASSIFIED','AIS-SPOOF-SUSPECT'),
  ('TRK-0003','AIR','UNKNOWN','ACTIVE',now() - INTERVAL 4 HOUR,
   59.20,-8.40,8000,420.0,45.0,0.67,2,'UNCLASSIFIED','UAV-UNKNOWN-001'),
  ('TRK-0004','AIR','FRIENDLY','ACTIVE',now() - INTERVAL 2 HOUR,
   60.10,-7.80,12000,480.0,220.0,0.98,3,'UNCLASSIFIED','VIPER-01'),
  ('TRK-0009','AIR','SUSPECT','ACTIVE',now() - INTERVAL 3 HOUR,
   58.90,-9.10,15000,650.0,180.0,0.76,2,'UNCLASSIFIED','CRUISE-SUSPECT-001'),
  ('TRK-0005','SUBSURFACE','TENTATIVE','ACTIVE',now() - INTERVAL 8 HOUR,
   57.50,-10.20,-150,8.0,120.0,0.51,1,'UNCLASSIFIED','ACOUSTIC-CONTACT-001'),
  ('TRK-0006','LAND','UNKNOWN','ACTIVE',now() - INTERVAL 10 HOUR,
   56.80,-8.90,150,15.0,90.0,0.73,1,'UNCLASSIFIED','VEHICLE-CONVOY-001'),
  ('TRK-0007','CYBER','UNKNOWN','ACTIVE',now() - INTERVAL 24 HOUR,
   0,0,0,0,0,0.88,1,'UNCLASSIFIED','IOC-C2-DOMAIN')" \
  "fused tracks"
...
SELECT
  ...
  multiIf(h BETWEEN 20 AND 24, 'SUSPECT', 'FRIENDLY') AS hostile_classification,
...
VALUES
  ('ALT-001','TRK-0001','SPEED_ANOMALY','CRITICAL',0.91,
   ...
   now() - INTERVAL 46 HOUR,'ACTIVE','UNCLASSIFIED'),
  ...
  ('ALT-003','TRK-0008','SPEED_ANOMALY','CRITICAL',0.94,
   ...
   now() - INTERVAL 12 HOUR,'ACTIVE','UNCLASSIFIED'),
  ('ALT-010','TRK-0010','AIS_MANIPULATION','CRITICAL',0.89,
   ...
   now() - INTERVAL 6 HOUR,'ACTIVE','UNCLASSIFIED'),
  ('ALT-004','TRK-0003','BEHAVIORAL_PATTERN','ELEVATED',0.78,
   ...
   now() - INTERVAL 4 HOUR,'ACTIVE','UNCLASSIFIED'),
  ('ALT-005','TRK-0009','ROUTE_DEVIATION','ELEVATED',0.81,
   ...
   now() - INTERVAL 3 HOUR,'ACTIVE','UNCLASSIFIED'),
  ('ALT-006','TRK-0005','TEMPORAL_ANOMALY','ELEVATED',0.69,
   ...
   now() - INTERVAL 8 HOUR,'ACTIVE','UNCLASSIFIED'),
  ...
  ('ALT-009','TRK-0002','BEHAVIORAL_PATTERN','ELEVATED',0.68,
   ...
   now() - INTERVAL 22 HOUR,'ACKNOWLEDGED','UNCLASSIFIED')" \
  "anomaly detections"
...
VALUES
  ('FB-001','OP-001','TRK-0002','ALT-008','CONFIRM_ANOMALY',0.85,
   ...
   now() - INTERVAL 23 HOUR,'UNCLASSIFIED'),
  ('FB-002','OP-001','TRK-0002','ALT-009','REJECT_ANOMALY',0.82,
   ...
   now() - INTERVAL 21 HOUR,'UNCLASSIFIED'),
  ('FB-003','OP-002','TRK-0001','ALT-001','CONFIRM_ANOMALY',0.79,
   ...
   now() - INTERVAL 45 HOUR,'UNCLASSIFIED'),
  ...
  ('FB-005','OP-999','TRK-0003','ALT-004','REJECT_ANOMALY',0.22,
   ...
   now() - INTERVAL 2 HOUR,'UNCLASSIFIED')" \
  "operator feedback"
...
VALUES
  ('AUD-001','svc-fusion-engine','TRACK_CREATED','svc-fusion-engine','SERVICE','track','TRK-0001',
   'CREATE',now() - INTERVAL 48 HOUR,'UNCLASSIFIED',
  ...
  ('AUD-005','svc-anomaly-detection','ALERT_CREATED','svc-anomaly-detection','SERVICE','alert','ALT-001',
   'CREATE',now() - INTERVAL 46 HOUR,'UNCLASSIFIED',
  ...
  ('AUD-008','svc-feedback','FEEDBACK_SUBMITTED','OP-001','OPERATOR','feedback','FB-002',
   'SUBMIT',now() - INTERVAL 21 HOUR,'UNCLASSIFIED',
  ...
  ('AUD-009','svc-feedback','ANTIPOISON_BLOCKED','svc-feedback','SERVICE','feedback','FB-005',
   'BLOCK',now() - INTERVAL 2 HOUR,'UNCLASSIFIED',
  ...
  ('AUD-010','svc-alert','ALERT_ASSIGNED','OP-001','OPERATOR','alert','ALT-003',
   'ASSIGN',now() - INTERVAL 11 HOUR,'UNCLASSIFIED',
  ...
  ('AUD-011','svc-nato-adapter','NATO_NOMINATED','OP-003','OPERATOR','nato_nomination','TRK-0002',
   'NOMINATE',now() - INTERVAL 20 HOUR,'UNCLASSIFIED',
  ...
  ('AUD-012','svc-nato-adapter','NATO_BLOCKED','svc-nato-adapter','SERVICE','nato_nomination','TRK-0008',
   'BLOCK',now() - INTERVAL 19 HOUR,'UNCLASSIFIED',
...
VALUES
  ('TRK-NATO-001','SURFACE','FRIENDLY','ACTIVE',now() - INTERVAL 30 HOUR,
   61.20,-5.80,0,18.5,135.0,0.99,1,'UNCLASSIFIED','NATO-ALLIED-FRIG-01'),
  ('TRK-NATO-002','SURFACE','FRIENDLY','ACTIVE',now() - INTERVAL 28 HOUR,
   60.80,-6.20,0,16.2,160.0,0.99,1,'UNCLASSIFIED','NATO-ALLIED-DEST-01'),
  ('TRK-NATO-003','AIR','FRIENDLY','ACTIVE',now() - INTERVAL 25 HOUR,
   61.50,-4.50,8500,380.0,270.0,0.99,1,'UNCLASSIFIED','NATO-ALLIED-P8-01'),
  ('TRK-NATO-004','AIR','FRIENDLY','ACTIVE',now() - INTERVAL 22 HOUR,
   60.90,-5.10,6000,350.0,290.0,0.99,1,'UNCLASSIFIED','NATO-ALLIED-MPA-02'),
  ('TRK-NATO-005','SURFACE','FRIENDLY','ACTIVE',now() - INTERVAL 18 HOUR,
   59.50,-6.80,0,12.0,180.0,0.99,1,'UNCLASSIFIED','NATO-ALLIED-SUP-01')" \
  "NATO allied tracks"
