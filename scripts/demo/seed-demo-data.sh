...
ch_exec() {
  ...
  response="$(curl -s -w "\n%{http_code}" "${http_url}" \
    -d "${sql}" \
    -u "${CLICKHOUSE_USER}:${CLICKHOUSE_PASSWORD}" 2>&1)"
  local http_code
  http_code="$(echo "$response" | tail -1)"
  local body
  body="$(echo "$response" | sed '$d')"
  if [ "$http_code" != "200" ]; then
    echo -e "${RED}[x]${NC} ClickHouse query failed: $label"
    echo -e "${RED}    HTTP ${http_code}: ${body}${NC}"
    exit 1
  fi
}

# ch_exec_soft — like ch_exec but does not exit on failure (for optional operations)
ch_exec_soft() {
  local sql="$1"
  local label="${2:-query}"
  if [ "$DRY_RUN" = "true" ]; then
...
ch_exec "INSERT INTO sensor_observations
  (observation_id, sensor_id, sensor_type, event_time, track_id,
   ...
   confidence, classification_level, metadata_json)
VALUES
  ('OBS-R-001','RADAR-NORTH-01','SENSOR_TYPE_RADAR',now() - INTERVAL 48 HOUR,'TRK-0001',
   60.21,-9.73,0,12.0,180.0,0.88,'CLASSIFICATION_LEVEL_UNCLASSIFIED',
   '{\"snr_db\":24.5,\"range_nm\":34.2,\"rcs_m2\":450}'),
  ('OBS-R-002','RADAR-NORTH-01','SENSOR_TYPE_RADAR',now() - INTERVAL 47 HOUR,'TRK-0001',
   60.09,-9.74,0,12.3,181.0,0.91,'CLASSIFICATION_LEVEL_UNCLASSIFIED',
   '{\"snr_db\":26.1,\"range_nm\":33.8,\"rcs_m2\":455}'),
  ('OBS-R-003','RADAR-SOUTH-02','SENSOR_TYPE_RADAR',now() - INTERVAL 46 HOUR,'TRK-0001',
   59.98,-9.75,0,28.5,182.0,0.79,'CLASSIFICATION_LEVEL_UNCLASSIFIED',
   '{\"snr_db\":19.2,\"range_nm\":67.4,\"rcs_m2\":462}'),
  ('OBS-EW-001','EW-STATION-01','SENSOR_TYPE_EW_SIGINT',now() - INTERVAL 48 HOUR,'TRK-0001',
   60.20,-9.72,100,0,0,0.84,'CLASSIFICATION_LEVEL_UNCLASSIFIED',
   '{\"frequency_ghz\":8.5,\"power_dbm\":42.3,\"modulation\":\"BURST\",\"emitter_id\":\"EW-UNK-001\"}'),
  ('OBS-EW-002','EW-STATION-03','SENSOR_TYPE_EW_SIGINT',now() - INTERVAL 47 HOUR,'TRK-0001',
   60.10,-9.74,100,0,0,0.81,'CLASSIFICATION_LEVEL_UNCLASSIFIED',
   '{\"frequency_ghz\":8.5,\"power_dbm\":38.7,\"modulation\":\"BURST\",\"emitter_id\":\"EW-UNK-001\"}'),
  ('OBS-EL-001','ELINT-ARRAY-01','SENSOR_TYPE_ELINT_COMINT',now() - INTERVAL 44 HOUR,'TRK-0005',
   57.50,-10.20,-50,0,0,0.72,'CLASSIFICATION_LEVEL_UNCLASSIFIED',
   '{\"emitter_id\":\"ELINT-SUB-001\",\"frequency_ghz\":0.15,\"intercept_type\":\"ACOUSTIC\",\"cep_m\":800}'),
  ('OBS-ISR-001','ISR-UAV-ALPHA','SENSOR_TYPE_ISR',now() - INTERVAL 36 HOUR,'TRK-0006',
   56.80,-8.90,0,15.0,90.0,0.95,'CLASSIFICATION_LEVEL_UNCLASSIFIED',
   '{\"platform_id\":\"UAV-ALPHA\",\"resolution_m\":0.5,\"coverage_area_km2\":6.25}'),
  ('OBS-AIS-001','AIS-COAST-01','SENSOR_TYPE_AIS_BFT',now() - INTERVAL 48 HOUR,'TRK-0002',
   60.50,-9.00,0,14.2,215.0,0.99,'CLASSIFICATION_LEVEL_UNCLASSIFIED',
   '{\"mmsi\":\"123456789\",\"vessel_name\":\"ALLIED-VESSEL-1\",\"vessel_type\":\"MILITARY\",\"nav_status\":\"UNDERWAY\"}'),
  ('OBS-AIS-002','AIS-COAST-01','SENSOR_TYPE_AIS_BFT',now() - INTERVAL 47 HOUR,'TRK-0002',
   60.38,-9.10,0,14.0,216.0,0.99,'CLASSIFICATION_LEVEL_UNCLASSIFIED',
   '{\"mmsi\":\"123456789\",\"vessel_name\":\"ALLIED-VESSEL-1\",\"vessel_type\":\"MILITARY\",\"nav_status\":\"UNDERWAY\"}'),
  ('OBS-AIS-003','AIS-COAST-01','SENSOR_TYPE_AIS_BFT',now() - INTERVAL 46 HOUR,'TRK-0001',
   61.50,-10.20,0,12.5,180.0,0.45,'CLASSIFICATION_LEVEL_UNCLASSIFIED',
   '{\"mmsi\":\"987654321\",\"vessel_name\":\"UNKNOWN-AIS\",\"vessel_type\":\"UNKNOWN\",\"spoof_flag\":true}'),
  ('OBS-CY-001','CYBER-FEED-01','SENSOR_TYPE_CYBER',now() - INTERVAL 24 HOUR,'TRK-0007',
   0,0,0,0,0,0.88,'CLASSIFICATION_LEVEL_UNCLASSIFIED',
...
ch_exec "INSERT INTO tracks_fused
  (track_id, entity_type, hostile_classification, track_status, event_time,
   latitude, longitude, altitude_meters, speed_knots, heading_degrees,
   confidence_score, source_count, classification_level, label)
VALUES
  ('TRK-0001','ENTITY_TYPE_SURFACE','HOSTILE_CLASSIFICATION_SUSPECT','TRACK_STATUS_ACTIVE',now() - INTERVAL 46 HOUR,
   59.98,-9.75,0,28.5,182.0,0.82,4,'CLASSIFICATION_LEVEL_UNCLASSIFIED','USV-SUSPECT-001'),
  ('TRK-0002','ENTITY_TYPE_SURFACE','HOSTILE_CLASSIFICATION_FRIENDLY','TRACK_STATUS_ACTIVE',now() - INTERVAL 47 HOUR,
   60.38,-9.10,0,14.0,216.0,0.95,2,'CLASSIFICATION_LEVEL_UNCLASSIFIED','ALLIED-VESSEL-1'),
  ('TRK-0008','ENTITY_TYPE_SURFACE','HOSTILE_CLASSIFICATION_HOSTILE','TRACK_STATUS_ACTIVE',now() - INTERVAL 12 HOUR,
   58.50,-11.20,0,32.1,90.0,0.91,3,'CLASSIFICATION_LEVEL_UNCLASSIFIED','FAST-MOVER-001'),
  ('TRK-0010','ENTITY_TYPE_SURFACE','HOSTILE_CLASSIFICATION_UNKNOWN','TRACK_STATUS_ACTIVE',now() - INTERVAL 6 HOUR,
   57.80,-10.50,0,8.5,270.0,0.83,2,'CLASSIFICATION_LEVEL_UNCLASSIFIED','AIS-SPOOF-SUSPECT'),
  ('TRK-0003','ENTITY_TYPE_AIR','HOSTILE_CLASSIFICATION_UNKNOWN','TRACK_STATUS_ACTIVE',now() - INTERVAL 4 HOUR,
   59.20,-8.40,8000,420.0,45.0,0.67,2,'CLASSIFICATION_LEVEL_UNCLASSIFIED','UAV-UNKNOWN-001'),
  ('TRK-0004','ENTITY_TYPE_AIR','HOSTILE_CLASSIFICATION_FRIENDLY','TRACK_STATUS_ACTIVE',now() - INTERVAL 2 HOUR,
   60.10,-7.80,12000,480.0,220.0,0.98,3,'CLASSIFICATION_LEVEL_UNCLASSIFIED','VIPER-01'),
  ('TRK-0009','ENTITY_TYPE_AIR','HOSTILE_CLASSIFICATION_SUSPECT','TRACK_STATUS_ACTIVE',now() - INTERVAL 3 HOUR,
   58.90,-9.10,15000,650.0,180.0,0.76,2,'CLASSIFICATION_LEVEL_UNCLASSIFIED','CRUISE-SUSPECT-001'),
  ('TRK-0005','ENTITY_TYPE_SUBSURFACE','HOSTILE_CLASSIFICATION_TENTATIVE','TRACK_STATUS_ACTIVE',now() - INTERVAL 8 HOUR,
   57.50,-10.20,-150,8.0,120.0,0.51,1,'CLASSIFICATION_LEVEL_UNCLASSIFIED','ACOUSTIC-CONTACT-001'),
  ('TRK-0006','ENTITY_TYPE_LAND','HOSTILE_CLASSIFICATION_UNKNOWN','TRACK_STATUS_ACTIVE',now() - INTERVAL 10 HOUR,
   56.80,-8.90,150,15.0,90.0,0.73,1,'CLASSIFICATION_LEVEL_UNCLASSIFIED','VEHICLE-CONVOY-001'),
  ('TRK-0007','ENTITY_TYPE_CYBER','HOSTILE_CLASSIFICATION_UNKNOWN','TRACK_STATUS_ACTIVE',now() - INTERVAL 24 HOUR,
   0,0,0,0,0,0.88,1,'CLASSIFICATION_LEVEL_UNCLASSIFIED','IOC-C2-DOMAIN')" \
  "fused tracks"
...
SELECT
  ...
  'ENTITY_TYPE_SURFACE' AS entity_type,
  multiIf(h BETWEEN 20 AND 24, 'HOSTILE_CLASSIFICATION_SUSPECT', 'HOSTILE_CLASSIFICATION_FRIENDLY') AS hostile_classification,
  'TRACK_STATUS_ACTIVE' AS track_status,
  now() - INTERVAL (96 - h * 2) HOUR AS event_time,
  60.50 + h * 0.02 * sin(h * 0.3) AS latitude,
  -9.00 + h * 0.03 * cos(h * 0.2) AS longitude,
  0 AS altitude_meters,
  ...
  2 AS source_count,
  'CLASSIFICATION_LEVEL_UNCLASSIFIED' AS classification_level,
...
VALUES
  ('ALT-001','TRK-0001','SPEED_ANOMALY','CRITICAL',0.91,
   ...
   now() - INTERVAL 46 HOUR,'ACTIVE','CLASSIFICATION_LEVEL_UNCLASSIFIED'),
  ...
  ('ALT-003','TRK-0008','SPEED_ANOMALY','CRITICAL',0.94,
   ...
   now() - INTERVAL 12 HOUR,'ACTIVE','CLASSIFICATION_LEVEL_UNCLASSIFIED'),
  ('ALT-010','TRK-0010','AIS_MANIPULATION','CRITICAL',0.89,
   ...
   now() - INTERVAL 6 HOUR,'ACTIVE','CLASSIFICATION_LEVEL_UNCLASSIFIED'),
  ('ALT-004','TRK-0003','BEHAVIORAL_PATTERN','ELEVATED',0.78,
   ...
   now() - INTERVAL 4 HOUR,'ACTIVE','CLASSIFICATION_LEVEL_UNCLASSIFIED'),
  ('ALT-005','TRK-0009','ROUTE_DEVIATION','ELEVATED',0.81,
   ...
   now() - INTERVAL 3 HOUR,'ACTIVE','CLASSIFICATION_LEVEL_UNCLASSIFIED'),
  ('ALT-006','TRK-0005','TEMPORAL_ANOMALY','ELEVATED',0.69,
   ...
   now() - INTERVAL 8 HOUR,'ACTIVE','CLASSIFICATION_LEVEL_UNCLASSIFIED'),
  ('ALT-007','TRK-0006','ROUTE_DEVIATION','ELEVATED',0.74,
   ...
   now() - INTERVAL 10 HOUR,'ACTIVE','CLASSIFICATION_LEVEL_UNCLASSIFIED'),
  ('ALT-008','TRK-0002','SPEED_ANOMALY','ELEVATED',0.73,
   'Vessel exceeded normal transit speed during 4-hour anomalous activity window.',
   now() - INTERVAL 24 HOUR,'ACKNOWLEDGED','CLASSIFICATION_LEVEL_UNCLASSIFIED'),
  ('ALT-009','TRK-0002','BEHAVIORAL_PATTERN','ELEVATED',0.68,
   ...
   now() - INTERVAL 22 HOUR,'ACKNOWLEDGED','CLASSIFICATION_LEVEL_UNCLASSIFIED')" \
  "anomaly detections"
...
VALUES
  ('FB-001','OP-001','TRK-0002','ALT-008','CONFIRM_ANOMALY',0.85,
   ...
   now() - INTERVAL 23 HOUR,'CLASSIFICATION_LEVEL_UNCLASSIFIED'),
  ('FB-002','OP-001','TRK-0002','ALT-009','REJECT_ANOMALY',0.82,
   ...
   now() - INTERVAL 21 HOUR,'CLASSIFICATION_LEVEL_UNCLASSIFIED'),
  ('FB-003','OP-002','TRK-0001','ALT-001','CONFIRM_ANOMALY',0.79,
   ...
   now() - INTERVAL 45 HOUR,'CLASSIFICATION_LEVEL_UNCLASSIFIED'),
  ...
  ('FB-005','OP-999','TRK-0003','ALT-004','REJECT_ANOMALY',0.22,
   'test_invalid_justification',
   now() - INTERVAL 2 HOUR,'CLASSIFICATION_LEVEL_UNCLASSIFIED')" \
  "operator feedback"
...
ch_exec "INSERT INTO audit_log
  (audit_id, service_id, event_type, actor_id, actor_type, resource_type, resource_id,
   action, event_time, classification_level, detail_json)
VALUES
  ('AUD-001','svc-fusion-engine','TRACK_CREATED','svc-fusion-engine','SERVICE','track','TRK-0001',
   'CREATE',now() - INTERVAL 48 HOUR,'CLASSIFICATION_LEVEL_UNCLASSIFIED',
   '{\"source_count\":4,\"initial_confidence\":0.82,\"entity_type\":\"SURFACE\"}'),
  ('AUD-002','svc-fusion-engine','TRACK_CREATED','svc-fusion-engine','SERVICE','track','TRK-0002',
   'CREATE',now() - INTERVAL 48 HOUR,'CLASSIFICATION_LEVEL_UNCLASSIFIED',
   '{\"source_count\":2,\"initial_confidence\":0.95,\"mmsi\":\"123456789\"}'),
  ('AUD-003','svc-fusion-engine','TRACK_MERGED','svc-fusion-engine','SERVICE','track','TRK-0001',
   'MERGE',now() - INTERVAL 38 HOUR,'CLASSIFICATION_LEVEL_UNCLASSIFIED',
   '{\"merged_from\":\"TRK-GHOST-001\",\"new_confidence\":0.87}'),
  ('AUD-004','svc-track','TRACK_STATUS_CHANGE','svc-track','SERVICE','track','TRK-GHOST-001',
   'STATUS_UPDATE',now() - INTERVAL 38 HOUR,'CLASSIFICATION_LEVEL_UNCLASSIFIED',
   '{\"previous_status\":\"ACTIVE\",\"new_status\":\"DROPPED\",\"reason\":\"merged\"}'),
  ('AUD-005','svc-anomaly-detection','ALERT_CREATED','svc-anomaly-detection','SERVICE','alert','ALT-001',
   'CREATE',now() - INTERVAL 46 HOUR,'CLASSIFICATION_LEVEL_UNCLASSIFIED',
   '{\"severity\":\"CRITICAL\",\"anomaly_type\":\"SPEED_ANOMALY\",\"track_id\":\"TRK-0001\"}'),
  ('AUD-006','svc-alert','ALERT_ACKNOWLEDGED','OP-001','OPERATOR','alert','ALT-008',
   'ACKNOWLEDGE',now() - INTERVAL 23 HOUR,'CLASSIFICATION_LEVEL_UNCLASSIFIED',
   '{\"operator_id\":\"OP-001\",\"track_id\":\"TRK-0002\"}'),
  ('AUD-007','svc-feedback','FEEDBACK_SUBMITTED','OP-001','OPERATOR','feedback','FB-001',
   'SUBMIT',now() - INTERVAL 23 HOUR,'CLASSIFICATION_LEVEL_UNCLASSIFIED',
   '{\"feedback_type\":\"CONFIRM_ANOMALY\",\"trust_score\":0.85,\"track_id\":\"TRK-0002\"}'),
  ('AUD-008','svc-feedback','FEEDBACK_SUBMITTED','OP-001','OPERATOR','feedback','FB-002',
   'SUBMIT',now() - INTERVAL 21 HOUR,'CLASSIFICATION_LEVEL_UNCLASSIFIED',
   '{\"feedback_type\":\"REJECT_ANOMALY\",\"trust_score\":0.82,\"track_id\":\"TRK-0002\"}'),
  ('AUD-009','svc-feedback','ANTIPOISON_BLOCKED','svc-feedback','SERVICE','feedback','FB-005',
   'BLOCK',now() - INTERVAL 2 HOUR,'CLASSIFICATION_LEVEL_UNCLASSIFIED',
   '{\"operator_id\":\"OP-999\",\"reason\":\"justification_too_short\",\"trust_score\":0.22}'),
  ('AUD-010','svc-alert','ALERT_ASSIGNED','OP-001','OPERATOR','alert','ALT-003',
   'ASSIGN',now() - INTERVAL 11 HOUR,'CLASSIFICATION_LEVEL_UNCLASSIFIED',
   '{\"assigner_operator_id\":\"OP-001\",\"assignee_operator_id\":\"OP-007\",\"comment\":\"Priority follow-up required\"}'),
  ('AUD-011','svc-nato-adapter','NATO_NOMINATED','OP-003','OPERATOR','nato_nomination','TRK-0002',
   'NOMINATE',now() - INTERVAL 20 HOUR,'CLASSIFICATION_LEVEL_UNCLASSIFIED',
   '{\"track_id\":\"TRK-0002\",\"format\":\"LINK16\",\"classification_check\":\"PASS\"}'),
  ('AUD-012','svc-nato-adapter','NATO_BLOCKED','svc-nato-adapter','SERVICE','nato_nomination','TRK-0008',
   'BLOCK',now() - INTERVAL 19 HOUR,'CLASSIFICATION_LEVEL_UNCLASSIFIED',
...
ch_exec "INSERT INTO tracks_fused
  (track_id, entity_type, hostile_classification, track_status, event_time,
   ...
   confidence_score, source_count, classification_level, label)
VALUES
  ('TRK-NATO-001','ENTITY_TYPE_SURFACE','HOSTILE_CLASSIFICATION_FRIENDLY','TRACK_STATUS_ACTIVE',now() - INTERVAL 30 HOUR,
   61.20,-5.80,0,18.5,135.0,0.99,1,'CLASSIFICATION_LEVEL_UNCLASSIFIED','NATO-ALLIED-FRIG-01'),
  ('TRK-NATO-002','ENTITY_TYPE_SURFACE','HOSTILE_CLASSIFICATION_FRIENDLY','TRACK_STATUS_ACTIVE',now() - INTERVAL 28 HOUR,
   60.80,-6.20,0,16.2,160.0,0.99,1,'CLASSIFICATION_LEVEL_UNCLASSIFIED','NATO-ALLIED-DEST-01'),
  ('TRK-NATO-003','ENTITY_TYPE_AIR','HOSTILE_CLASSIFICATION_FRIENDLY','TRACK_STATUS_ACTIVE',now() - INTERVAL 25 HOUR,
   61.50,-4.50,8500,380.0,270.0,0.99,1,'CLASSIFICATION_LEVEL_UNCLASSIFIED','NATO-ALLIED-P8-01'),
  ('TRK-NATO-004','ENTITY_TYPE_AIR','HOSTILE_CLASSIFICATION_FRIENDLY','TRACK_STATUS_ACTIVE',now() - INTERVAL 22 HOUR,
   60.90,-5.10,6000,350.0,290.0,0.99,1,'CLASSIFICATION_LEVEL_UNCLASSIFIED','NATO-ALLIED-MPA-02'),
  ('TRK-NATO-005','ENTITY_TYPE_SURFACE','HOSTILE_CLASSIFICATION_FRIENDLY','TRACK_STATUS_ACTIVE',now() - INTERVAL 18 HOUR,
   59.50,-6.80,0,12.0,180.0,0.99,1,'CLASSIFICATION_LEVEL_UNCLASSIFIED','NATO-ALLIED-SUP-01')" \
  "NATO allied tracks"
...
for view in mv_active_tracks_by_domain mv_sensor_throughput_5min mv_alert_ack_latency; do
  if ch_exec_soft "OPTIMIZE TABLE ${view} FINAL" "optimize ${view}"; then
    log_pass "  ${view} optimized"
  else
    log_warn "  ${view} not yet created — run bash scripts/dev/init-clickhouse.sh first"
  fi
done

log_pass "Materialized view optimization complete"
