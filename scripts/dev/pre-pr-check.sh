...
gate_1_pre_build() {
  ...
  if [ -d "${REPO_ROOT}/web-cop-gpu" ] && command -v pnpm &>/dev/null; then
    if (cd "${REPO_ROOT}/web-cop-gpu" && pnpm prettier --check . 2>/dev/null); then
...
gate_2_build() {
  ...
  if [ -d "${REPO_ROOT}/web-cop-gpu" ] && command -v pnpm &>/dev/null; then
    if (cd "${REPO_ROOT}/web-cop-gpu" && pnpm tsc --noEmit 2>/dev/null); then
...
gate_3_test() {
  ...
  # Frontend tests
  if [ -d "${REPO_ROOT}/web-cop-gpu" ] && command -v pnpm &>/dev/null; then
    if (cd "${REPO_ROOT}/web-cop-gpu" && pnpm test --coverage --run 2>/dev/null); then
...
gate_4_security() {
  ...
  if [ -d "${REPO_ROOT}/web-cop-gpu" ] && command -v pnpm &>/dev/null; then
    if (cd "${REPO_ROOT}/web-cop-gpu" && pnpm audit --audit-level=high 2>/dev/null); then
      ...
      log_info "  Run: cd web-cop-gpu && pnpm audit --fix"
    fi
  fi
}
