...
// NewIngestionHandler creates a new Cyber IOC ingestion handler.
...
// IngestSingleObservation handles unary Cyber observation ingestion.
...
// GetSensorStatus returns live statistics for the Cyber ingestion service.
...
func (h *IngestionHandler) ListSensorStatuses(ctx context.Context, req *ingestionv1.ListSensorStatusesRequest) (*ingestionv1.ListSensorStatusesResponse, error) {
	...
	if len(sensors) == 0 && req.ActiveWithinSeconds == 0 {
		resp, err := h.GetSensorStatus(ctx, &ingestionv1.GetSensorStatusRequest{SensorId: "cyber-cluster-01"})
		if err != nil {
...
// GetSensorDiagnostic returns deep diagnostic data for a specific Cyber sensor.
