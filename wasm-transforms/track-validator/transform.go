...
var requiredTrackHeaders = []string{
	"rtsa-classification",
	"rtsa-source-service",
	"rtsa-trace-id",
	"rtsa-timestamp",
...
// validateTrackMessage runs on every record written to tracks.fused.* topics.
...
func validateTrackMessage(event transform.WriteEvent, writer transform.RecordWriter) error {
	record := event.Record()

	// Validate required headers.
	for _, h := range requiredTrackHeaders {
	...
	if track.GetTrackId() == "" {
		return writeToDLQ(writer, record, "missing track_id")
	}
...
func writeToDLQ(writer transform.RecordWriter, record transform.Record, reason string) error {
	...
	return writer.Write(record, transform.ToTopic("dlq.tracks"))
}
