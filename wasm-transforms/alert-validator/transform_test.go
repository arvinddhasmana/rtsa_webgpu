...
func validHeaders() []transform.RecordHeader {
	return []transform.RecordHeader{
		{Key: []byte("rtsa-classification"), Value: []byte("PROTECTED_B")},
		{Key: []byte("rtsa-source-service"), Value: []byte("svc-anomaly-detection")},
		{Key: []byte("rtsa-trace-id"), Value: []byte("trace-xyz-456")},
		{Key: []byte("rtsa-timestamp"), Value: []byte("2026-02-24T07:15:00Z")},
		{Key: []byte("rtsa-schema-version"), Value: []byte("1")},
	}
}

func validAlertProto(t *testing.T) []byte {
	t.Helper()
	alert := &inferencev1.AnomalyAlert{AlertId: "ALERT-001"}
...
func TestValidateAlertMessage_ValidMessage_PassesThrough(t *testing.T) {
	w := &recordingWriter{}
	e := &mockWriteEvent{record: transform.Record{
		Headers: validHeaders(),
		Value:   validAlertProto(t),
	}}
	if err := validateAlertMessage(e, w); err != nil {
...
func TestValidateAlertMessage_MissingClassification_RoutedToDLQ(t *testing.T) {
	w := &recordingWriter{}
	e := &mockWriteEvent{record: transform.Record{
...
func TestValidateAlertMessage_InvalidClassification_RoutedToDLQ(t *testing.T) {
	headers := validHeaders()
	for i, h := range headers {
		if string(h.Key) == "rtsa-classification" {
			headers[i].Value = []byte("CONFIDENTIAL")
		}
	}
	w := &recordingWriter{}
	e := &mockWriteEvent{record: transform.Record{Headers: headers, Value: validAlertProto(t)}}
	if err := validateAlertMessage(e, w); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if w.routedTo == "" {
...
func TestValidateAlertMessage_EmptyValue_RoutedToDLQ(t *testing.T) {
	w := &recordingWriter{}
...
func TestValidateAlertMessage_InvalidProtobuf_RoutedToDLQ(t *testing.T) {
	w := &recordingWriter{}
	e := &mockWriteEvent{record: transform.Record{
		Headers: validHeaders(),
...
func TestValidateAlertMessage_EmptyAlertID_RoutedToDLQ(t *testing.T) {
	alert := &inferencev1.AnomalyAlert{AlertId: ""}
	...
	if w.routedTo == "" {
		t.Error("expected DLQ routing for empty alert_id")
	}
}

func TestValidateAlertMessage_EachRequiredHeaderMissing_RoutedToDLQ(t *testing.T) {
	for _, missing := range requiredAlertHeaders {
