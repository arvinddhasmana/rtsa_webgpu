...
## 5. Pipeline & Demo Infrastructure Fixes (Post v2.0 QA)

The following fixes were applied after QA revealed a "No Tracks — awaiting data stream" failure on the Fusion Dashboard when running demo scripts. Root-cause analysis identified 12+ bugs spanning the full pipeline (Simulator → gRPC → Ingestion → Redpanda → WASM Transforms → Fusion Engine → svc-track → Envoy → Browser).

### 5.1 Demo Script Orchestration

| Item                                | Artifact                           | Change                                                                                                                                                                                                                |
| ----------------------------------- | ---------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Shared library for all demo scripts | `scripts/demo/_common.sh`          | **Created.** 4-phase orchestrated startup: (1) infrastructure, (2) topic/schema init, (3) application services, (4) health-check polling via `wait_for_service_health()`. Replaces hardcoded `sleep 30`.              |
| All 8 demo scripts rewritten        | `scripts/demo/run-*.sh`            | **Rewritten.** Each now sources `_common.sh`, uses `parse_common_args` (--seed/--dry-run/--skip-infra), and calls `start_infrastructure_and_services()`.                                                              |
| 5 missing scenario YAMLs            | `tools/simulator/scenarios/*.yaml` | **Created.** `fusion-dashboard-demo.yaml`, `operator-ui-demo.yaml`, `sensor-health-demo.yaml`, `nato-exchange-demo.yaml`, `analyst-forensics-demo.yaml`. Missing files caused simulator crash for non-maritime demos. |

### 5.2 Docker Compose Health Checks

| Item                             | Artifact                             | Change                                                                                                                                                                                           |
| -------------------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Health checks on all 15 services | `deploy/docker-compose.services.yml` | **Added.** Every `svc-*` container now has `healthcheck: test: ["CMD", "wget", "-qO-", "http://localhost:8081/healthz"]` with `interval: 10s, timeout: 5s, retries: 5, start_period: 30s`.       |
| Simulator depends_on condition   | `deploy/docker-compose.services.yml` | **Changed.** Simulator `depends_on` now uses `condition: service_healthy` for all 6 ingestion services instead of bare service names. Prevents data loss from sending before services are ready. |

### 5.3 Fusion Engine Fixes

| Item                              | Artifact                                                | Change                                                                                                                                                                                                                                                                                         |
| --------------------------------- | ------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Cross-domain gating bug           | `svc-fusion-engine/internal/domain/gating.go`           | **Fixed.** `sensorTypeToEntityType()` now returns `ENTITY_TYPE_UNSPECIFIED` for `SENSOR_TYPE_EW_SIGINT` and `SENSOR_TYPE_ELINT_COMINT` (was incorrectly returning `ENTITY_TYPE_AIR`). `entityTypesCompatible()` treats UNSPECIFIED as wildcard, enabling cross-domain correlation as designed. |
| Missing speed/heading computation | `svc-fusion-engine/internal/domain/track_manager.go`    | **Fixed.** `ToFusedTrack()` now computes `speedKnots = √(VelocityN² + VelocityE²) / 0.514444` and `headingDeg = atan2(VelocityE, VelocityN) × 180/π` (normalized 0–360). Frontend was showing 0 knots / 0° for all tracks.                                                                     |
| Silent produce failure            | `svc-fusion-engine/internal/handler/fusion_pipeline.go` | **Fixed.** Replaced `slog.Warn("produce failed", ...)` + continue with `return fmt.Errorf(...)`. Fused tracks were silently dropped on Redpanda produce errors.                                                                                                                                |

### 5.4 Encoding Consistency (protojson)

| Item                                      | Artifact                                                               | Change                                                                                                                                                                                                                                                                        |
| ----------------------------------------- | ---------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| WASM transforms: proto binary → protojson | `wasm-transforms/{sensor,alert,feedback,track}-validator/transform.go` | **Fixed (4 files).** Changed `proto.Unmarshal` to `protojson.Unmarshal`. Redpanda messages are JSON (produced via `protojson.MarshalOptions{UseProtoNames: true}`); binary unmarshal silently returned zero-value messages, causing all WASM validators to pass invalid data. |
| Unit test encoding                        | `svc-fusion-engine/internal/handler/fusion_pipeline_test.go`           | **Fixed.** `makeRecord()` now uses `protojson.MarshalOptions{UseProtoNames: true}.Marshal(obs)` instead of `proto.Marshal(obs)`.                                                                                                                                              |
| Integration test encoding + topic         | `svc-fusion-engine/internal/integration/fusion_integration_test.go`    | **Fixed.** Encoding changed to protojson. Topic changed from `"sensors.radar"` to `"sensors.radar.tracks"` (matching `RTSA_FUSION_INPUT_TOPICS`).                                                                                                                             |

### 5.5 Seed Data Enum Normalization

| Item                         | Artifact                         | Change                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ---------------------------- | -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| All enum strings in seed SQL | `scripts/demo/seed-demo-data.sh` | **Fixed (all 7 data sections).** ClickHouse `LowCardinality(String)` columns now use full proto enum names: `SENSOR_TYPE_RADAR` (was `RADAR`), `ENTITY_TYPE_SURFACE` (was `SURFACE`), `HOSTILE_CLASSIFICATION_SUSPECT` (was `SUSPECT`), `TRACK_STATUS_ACTIVE` (was `ACTIVE`), `CLASSIFICATION_LEVEL_UNCLASSIFIED` (was `UNCLASSIFIED`). Required because `svc-query` uses `EntityType_value[entityType]` map lookups — short names returned 0 (UNSPECIFIED), silently corrupting all historical query results. |

### 5.6 Simulator Error Handling

| Item                         | Artifact                                | Change                                                                                                                                                                                                                                                                                                                                                                                                       |
| ---------------------------- | --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Consecutive failure tracking | `tools/simulator/cmd/simulator/main.go` | **Fixed.** Added atomic consecutive failure counter. After 10+ consecutive gRPC send failures, log level escalates from Warn → Error with message "gRPC sends failing — are ingestion services running?". Counter resets on any successful send. `runTick()` now logs a summary with sent/failed counts when any failures occur. Previously, the simulator appeared "successful" even when all sends failed. |

### 5.7 Documentation Fixes

| Item                      | Artifact                 | Change                                                                                                                                                                                      |
| ------------------------- | ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Envoy README port numbers | `deploy/envoy/README.md` | **Fixed.** Route Summary table backend ports corrected from host-mapped ports (50070–50073) to the internal container port 50051 that Envoy actually connects to within the Docker network. |
