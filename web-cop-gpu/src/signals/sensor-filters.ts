...
export const [selectedStatuses, setSelectedStatuses] = createSignal<string[]>(["CONNECTED", "STALE", "OFFLINE"]);

/** Active sensor type filters. */
export const [selectedTypes, setSelectedTypes] = createSignal<string[]>(["RADAR", "EW/SIGINT", "ELINT/COMINT", "ISR", "AIS/BFT", "CYBER"]);
...
export function toggleStatusFilter(status: string) {
    ...
    if (current.includes(status)) {
        if (current.length > 1) { // Don't allow empty filters
