...
"github.com/arvinddhasmana/rtsa_webgpu/svc-radar-ingestion/internal/handler"
"github.com/arvinddhasmana/rtsa_webgpu/svc-radar-ingestion/internal/mapper"
...
if err != nil {
// Use a temporary logger before telemetry is up
...
auditEmitter := audit.NewEmitter(prod, "svc-radar-ingestion", logger)
...
domain.WithMaxSurfaceSpeedKnots(cfg.MaxSurfaceSpeedKnots),
domain.WithMaxAirSpeedKnots(cfg.MaxAirSpeedKnots),
domain.WithMaxFutureOffset(time.Duration(cfg.MaxFutureOffsetSec)*time.Second),
domain.WithMaxPastOffset(time.Duration(cfg.MaxPastOffsetSec)*time.Second),
)
...
meter := tp.MeterProvider.Meter("svc-radar-ingestion")
...
zap.String("service", "svc-radar-ingestion"),
