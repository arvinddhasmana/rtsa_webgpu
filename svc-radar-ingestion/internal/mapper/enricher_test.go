...
e := mapper.NewEnricher("svc-radar-ingestion", guard)
...
if obs.Metadata["rtsa.source_service"] != "svc-radar-ingestion" {
t.Errorf("expected rtsa.source_service to be set, got: %s",
obs.Metadata["rtsa.source_service"])
}
...
e := mapper.NewEnricher("svc-test", nil)

obs := &ingestionv1.SensorObservation{
Classification: commonv1.ClassificationLevel_CLASSIFICATION_LEVEL_SECRET,
}

// Should not error with nil guard
if err := e.Enrich(context.Background(), obs); err != nil {
...
if obs.Metadata["rtsa.source_service"] == "" {
t.Error("expected source_service to be set")
}
}
