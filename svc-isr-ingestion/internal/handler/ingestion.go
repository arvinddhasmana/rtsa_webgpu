...
"github.com/arvinddhasmana/RTSA_VS_Opus/svc-isr-ingestion/internal/producer"
"go.uber.org/zap"
...
// IngestionHandler implements IngestionService for ISR data.
...
// NewIngestionHandler creates a new ISR ingestion handler.
...
// IngestSingleObservation handles unary ISR observation ingestion.
...
// IngestSensorData handles client-streaming ISR observation ingestion.
...
// GetSensorStatus returns live statistics for the ISR ingestion service.
...
// ListSensorStatuses returns a list of all active ISR sensor statistics.
func (h *IngestionHandler) ListSensorStatuses(ctx context.Context, req *ingestionv1.ListSensorStatusesRequest) (*ingestionv1.ListSensorStatusesResponse, error) {
	...
	if len(sensors) == 0 && req.ActiveWithinSeconds == 0 {
		resp, err := h.GetSensorStatus(ctx, &ingestionv1.GetSensorStatusRequest{SensorId: "isr-cluster-01"})
		if err != nil {
...
// GetSensorDiagnostic returns deep diagnostic data for a specific ISR sensor.
