...
import (
	...
	"github.com/arvinddhasmana/rtsa_webgpu/svc-ew-ingestion/internal/mapper"
	"github.com/arvinddhasmana/rtsa_webgpu/svc-ew-ingestion/internal/producer"
	"go.uber.org/zap"
...
// IngestionHandler implements IngestionService for EW/SIGINT data.
...
// NewIngestionHandler creates a new EW/SIGINT ingestion handler.
...
// IngestSingleObservation handles unary EW observation ingestion.
...
zap.String("sensor_id", obs.GetSensorId()),
zap.String("observation_id", obs.ObservationId),
zap.String("sensor_type", "EW/SIGINT"),
...
// IngestSensorData handles client-streaming EW observation ingestion.
...
// GetSensorStatus returns live statistics for the EW ingestion service.
...
// ListSensorStatuses returns a list of all active EW/SIGINT sensor statistics.
func (h *IngestionHandler) ListSensorStatuses(ctx context.Context, req *ingestionv1.ListSensorStatusesRequest) (*ingestionv1.ListSensorStatusesResponse, error) {
	...
	if len(sensors) == 0 && req.ActiveWithinSeconds == 0 {
		resp, err := h.GetSensorStatus(ctx, &ingestionv1.GetSensorStatusRequest{SensorId: "ew-cluster-01"})
		if err != nil {
...
// GetSensorDiagnostic returns deep diagnostic data for a specific EW/SIGINT sensor.
