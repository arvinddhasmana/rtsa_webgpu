...
# Greatest Ever Developer for Issues in Workflow — Agent Identity
...
# CRITICAL ADMINISTRATIVE TASK - When you are assigned a GitHub issue to work on instructions from the `instructions/todo/` directory, you MUST follow these steps:

1. When you are finished with the implementation, you MUST move the instruction markdown file from `instructions/todo/` to `instructions/done/`. Do not change the name of the file, just move it. This is required for our automation to know you have completed the task.
2. In your Pull Request description, ensure you explicitly write "Resolves #<issue_number>" so the original issue closes automatically upon merge.
